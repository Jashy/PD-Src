proc BUFF2INV {cells} {
	foreach c [get_attr [get_flat_cells $cells] full_name] {	
		...
		remove_cell $ful_name
                if {[regexp  {CKBD(\d*)} $re_name a var1] == 1} {
			regsub {CKBD(\d*)} $re_name CKND${var1} new_ref_name
		        create_cell $ful_name $new_ref_name
	        } elseif {[regexp  {BUFFD(\d*)} $re_name a var1] == 1} {
                         regsub {BUFFD(\d*)} $re_name INVD${var1} new_ref_name
		         create_cell $ful_name $new_ref_name
        	} else { puts "Error ....."}

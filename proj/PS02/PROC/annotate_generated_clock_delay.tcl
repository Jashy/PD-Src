  ...
  foreach_in_collection clock $clocks {
    set source_name [ get_attr [ get_attr $clock sources ] full_name ]
    if { [ sizeof_collection [ get_ports $source_name -quiet ] ] == 1 } { continue }
    if { [ string match "UUruby_dft/ruby_core/image_proc/*" $source_name ] == 1 } { continue }
    if { [ string match "UUruby_dft/ruby_core/arm926/*"     $source_name ] == 1 } { continue }
    set pin_names [ concat $pin_names $source_name ]
  }
  ...
  foreach pin_name $pin_names {
    ...
    if { $is_macro == 0 } {
      puts "INFO: INSERT_BUFFER $input_pin_name CLKBUFV12 -place"
...
#  UUruby_dft/ruby_core/usb_otg_1/u_otg_U_DWC_otg_mac_U_DWC_otg_mac_pie_xfer_resp_reg_0_/CK 0.411 -0.000 0.120 rp-+
  ...
  while { [ gets $of line ] >= 0 } {
    if { [ regexp {^\s+Clock:\s+CTS_(\S+)} $line "" clock_name ] == 1 } { }
    if { [ regexp {^\s+\S+\s+(\S+)\s+w\Sp} $line "" max ] == 1 }  { }

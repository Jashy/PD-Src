...
proc DUPLICATE_ICG {root_pin_names { fanout_limit 60 } { cluster_size_x 30 }  { cluster_size_y 70 }  } {
...
# Modified by Darnell Wei on 14/11/2015
    #set root_cell_model [ get_model [ get_attr $root_cell ref_name ] ]
    #set root_cell_model  [get_attr $root_cell ref_name ]
    set root_cell_model [ get_model LVLUO_X4M_A8TR ]
    ...
    foreach matrix_index [ array names pins_by_matrix_index ] {
      ...
      while { [ llength $tmp_pins ] > 0 } {
        ...
        foreach_in_collection root_input_pin [ get_pins -of $root_cell -filter " direction == in " ] {
          ...
          set root_input_net [ get_nets -of $root_input_pin ]
        ...
        # Move cells
        move_objects $new_gating_cell_name -x $location_x -y $location_y -ignore_fixed
